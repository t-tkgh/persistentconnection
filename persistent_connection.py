...
class CallbackModule(CallbackBase):
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        for index, host in enumerate(Dict):
            inventory_data[index] = list(Dict.values())[index]
    ...
    def v2_playbook_on_task_start(self, task, **kwargs):
        ...
        maxindex = list(inventory_data.keys())
        for index in maxindex:
            sshport = 22

            if "jobid" in str(list(vars(task._loader)["_FILE_CACHE"].values())[0][0]['vars']):
                jobid = list(vars(task._loader)["_FILE_CACHE"].values())[
                    0][0]['vars']['jobid']
            ...
            m.update((str(inventory_data[index]['inventory_hostname']) + '-'
                     + str(sshport) + '-' + str(list(vars(task._loader)["_FILE_CACHE"].values())[0][0]['remote_user'])
                     + '-' + 'network_cli' + '-' + str(pid)).encode('utf-8'))
            digest = m.hexdigest()
            cpath = digest[:10]

            filename_dict.update({index: {'host': inventory_data[index]['inventory_hostname'],
                 'sshport': sshport, 'remote_user': list(vars(task._loader)["_FILE_CACHE"].values())[0][0]['remote_user'],
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        files = os.listdir(tmpdir)
        for file in files:
            for index in list(filename_dict.keys()):
                socketfile = re.compile(
